...
void GetOtherCodePinYin(int nCode, string& strValue);
 #include <string.h>
void GetPinYin(unsigned char* Chinese, string& PinYin, double& chinese_count)
{

    int chinese_str_len = strlen((char*)Chinese);
    for (int i = 0; i < chinese_str_len; i++)
    {
        if (Chinese[i] > 0 && Chinese[i] < 160)//对于文章中的英文直接存入尾部
        {
            PinYin += Chinese[i];
        }
        else
        {
            ...
            ++i;
            if (ascii_code < 1601) //未知字符s，不做输出
            {
                PinYin += "";
                continue;
            }
            chinese_count +=1;
            if (ascii_code > 5589)//对于其他编码方法的处理
            {
                string s;
            ...
            for (int j = 396; j >= 0; j--) //寻找第二级汉字编码（56-87区的汉字），将对应的拼音存入
            {
...
void GetPinYinCount(unsigned char* pinyin_input, double* f)//每个按键出现次数统计
{

    int pinyin_input_str_len = strlen((char*)pinyin_input);
    for (int i = 0; i < pinyin_input_str_len; i++)
    {

        switch(pinyin_input[i]){
            case 'a':f[0] += 1;break;
            case 'b':f[1] += 1;break;
            case 'c':f[2] += 1;break;
            case 'd':f[3] += 1;break;
...
void getMeanStd0(double *f, double sum)//方差计算
{
    double sq_sum=0;
    for(int i=0;i<26;i++)
    {
        sq_sum += pow(f[i]/sum-1/26,2);
    }
    cout<<"variance  is:"<<setiosflags(ios::fixed)<<setprecision(10)<<sq_sum/26<<endl;

}
void Statistic(ifstream &input, ofstream &output,double chinese_count){//每个字母按键出现频率统计
    ...
    if(input){
        while(getline(input, pinyin_input)){
                GetPinYinCount((unsigned char*)pinyin_input.c_str(), f);
        }
    }
    else // 没有该文件
    {
    ...
    for(int i = 0;i<26; i++){
     ...
     output<<"a:"<<f[0]/sum<<endl;
     output<<"b:"<<f[1]/sum<<endl;
     output<<"c:"<<f[2]/sum<<endl;
     output<<"d:"<<f[3]/sum<<endl;
     output<<"e:"<<f[4]/sum<<endl;
     output<<"f:"<<f[5]/sum<<endl;
     output<<"g:"<<f[6]/sum<<endl;
     output<<"h:"<<f[7]/sum<<endl;
     output<<"i:"<<f[8]/sum<<endl;
     output<<"j:"<<f[9]/sum<<endl;
     output<<"k:"<<f[10]/sum<<endl;
     output<<"l:"<<f[11]/sum<<endl;
     output<<"m:"<<f[12]/sum<<endl;
     output<<"n:"<<f[13]/sum<<endl;
     output<<"o:"<<f[14]/sum<<endl;
     output<<"p:"<<f[15]/sum<<endl;
     output<<"q:"<<f[16]/sum<<endl;
     output<<"r:"<<f[17]/sum<<endl;
     output<<"s:"<<f[18]/sum<<endl;
     output<<"t:"<<f[19]/sum<<endl;
     output<<"u:"<<f[20]/sum<<endl;
     output<<"v:"<<f[21]/sum<<endl;
     output<<"w:"<<f[22]/sum<<endl;
     output<<"x:"<<f[23]/sum<<endl;
     output<<"y:"<<f[24]/sum<<endl;
     output<<"z:"<<f[25]/sum<<endl;
     cout<<"已生成statistics文件"<<endl;

     getMeanStd0(f,sum);

     //平均码长计算
     cout<<"average lengh of code is:"<<setiosflags(ios::fixed)<<setprecision(10)<<sum/chinese_count<<endl;
     //cout<<chinese_count<<endl;
...
bool FileStringReplace(ifstream &instream, ofstream &outstream)
{
	...
	{
		pos_ei = str.find("ei"); // 查找每一行中的"ei"
		while(str.find("ei") != string::npos)
		{
			str = str.replace(pos_ei, 2, "v"); // 将ei替换为v
			pos_ei = str.find("ei"); // 查找每一行中的"ei"
			//outstream << str << endl;
			//continue;
		}
		pos_an=str.find("an");
		while(str.find("an") != string::npos)
		{
			str = str.replace(pos_an, 2, "p"); // 将ei替换为v
			pos_an = str.find("an"); // 查找每一行中的"an"
			//outstream << str << endl;
			//continue;
		}
		pos_en=str.find("en");
		while(str.find("en") != string::npos)
		{
			str = str.replace(pos_en, 2, "k"); // 将ei替换为v
			pos_en = str.find("en"); // 查找每一行中的"en"
			//outstream << str << endl;
			//continue;
		}
...
int main(void)
{
    ...
    string chinese;
    double chinese_count = 0;


    if(in){
        while(getline(in, chinese, '\n')){

                string pinyin;
                GetPinYin((unsigned char*)chinese.c_str(), pinyin,chinese_count);
                out<<pinyin<<endl;
    ...
    in.close();
    out.close();
    ifstream in_ex("output.txt");
    ofstream out_ex("exchange.txt");



    FileStringReplace(in_ex, out_ex);
    in_ex.close();
    out_ex.close();

    ifstream in_exx("output.txt");
    ofstream statistics("statistics.txt");
    Statistic(in_exx,statistics,chinese_count);
    statistics.close();


    ifstream inexchange("exchange.txt");
    ofstream statistics_ex("statistics_ex.txt");
    Statistic(inexchange,statistics_ex,chinese_count);
